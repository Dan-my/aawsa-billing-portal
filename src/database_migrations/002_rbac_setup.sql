...
-- The SECURITY DEFINER clause is crucial. It allows the function to run with the permissions
-- of the user who defined it (the database admin), not the user calling it (the app user).
CREATE OR REPLACE FUNCTION public.update_role_permissions(p_role_id bigint, p_permission_ids integer[])
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
COMMENT ON FUNCTION public.update_role_permissions(bigint, integer[]) IS 'Updates the permissions for a given role by deleting existing ones and inserting the new set.';
