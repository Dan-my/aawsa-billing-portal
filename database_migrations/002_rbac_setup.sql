...
-- It relies on `auth.uid()` which is securely provided by Supabase.
...
-- CORRECTED: Allow ANY user (including anonymous) to read the list of roles.
CREATE POLICY "Allow public read access" ON public.roles FOR SELECT USING (true);
-- SECURE: Only allow users with the 'permissions_update' permission to modify roles.
CREATE POLICY "Allow admin modification" ON public.roles FOR ALL USING (public.user_has_permission('permissions_update')) WITH CHECK (public.user_has_permission('permissions_update'));

-- Policies for 'permissions' table
-- CORRECTED: Allow ANY user to read the list of available permissions.
CREATE POLICY "Allow public read access" ON public.permissions FOR SELECT USING (true);
-- SECURE: Only allow users with the 'permissions_update' permission to modify permissions.
CREATE POLICY "Allow admin modification" ON public.permissions FOR ALL USING (public.user_has_permission('permissions_update')) WITH CHECK (public.user_has_permission('permissions_update'));

-- Policies for 'role_permissions' join table
-- CORRECTED: Allow ANY user to read the links between roles and permissions.
CREATE POLICY "Allow public read access" ON public.role_permissions FOR SELECT USING (true);
-- SECURE: Explicitly block ALL direct insert, update, or delete operations on this table.
...
-- ========= Section 4: Initial Data Seeding and Functions (Safe to re-run) =========
