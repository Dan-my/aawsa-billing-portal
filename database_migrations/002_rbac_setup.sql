...
-- Allows any authenticated user to read the list of roles.
CREATE POLICY "Allow public read access" ON public.roles FOR SELECT TO authenticated USING (true);
-- Allows users with the 'permissions_update' permission to do everything else.
CREATE POLICY "Allow admin modification" ON public.roles FOR ALL TO authenticated USING (public.user_has_permission('permissions_update')) WITH CHECK (public.user_has_permission('permissions_update'));

-- Policies for 'permissions' table
-- Allows any authenticated user to read the list of permissions.
CREATE POLICY "Allow public read access" ON public.permissions FOR SELECT TO authenticated USING (true);
-- Allows users with the 'permissions_update' permission to do everything else.
CREATE POLICY "Allow admin modification" ON public.permissions FOR ALL TO authenticated USING (public.user_has_permission('permissions_update')) WITH CHECK (public.user_has_permission('permissions_update'));

-- Policies for 'role_permissions' join table
-- Allows any authenticated user to read the links between roles and permissions.
CREATE POLICY "Allow public read access" ON public.role_permissions FOR SELECT TO authenticated USING (true);
-- IMPORTANT: Explicitly blocks ALL direct insert, update, or delete operations on this table.
-- All changes MUST go through the secure `update_role_permissions` database function.
CREATE POLICY "Disallow direct modification" ON public.role_permissions FOR ALL TO authenticated USING (false) WITH CHECK (false);
