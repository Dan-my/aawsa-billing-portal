...
const createMeterRentSchema = (prices: { [key: string]: number }) => {
  const schemaObject = Object.keys(prices).reduce((acc, key) => {
    acc[`price_${key.replace('.', '_')}`] = z.coerce.number().min(0, "Price must be non-negative.");
    return acc;
...
export function MeterRentDialog({ open, onOpenChange, onSubmit, defaultPrices, currency = "ETB" }: MeterRentDialogProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: Object.entries(defaultPrices).reduce((acc, [key, value]) => {
      acc[`price_${key.replace('.', '_')}`] = value;
      return acc;
    }, {} as any),
  });

  React.useEffect(() => {
    // Reset form if default prices change (e.g., reset to default)
  ...
  const handleSubmit = (data: FormValues) => {
    ...
    const newPrices = Object.entries(data).reduce((acc, [formKey, value]) => {
      const originalKey = formKey.replace('price_', '').replace('_', '.');
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Manage Meter Rent Prices</DialogTitle>
          <DialogDescription>
            Update the monthly rent price for each meter size. These prices are used in all bill calculations.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
                ...
                <div className="space-y-4">
                  {meterSizeOptions.map(option => (
                      <FormField
                          ...
                          name={`price_${option.value.replace('.', '_')}`}
                          render={({ field }) => (
